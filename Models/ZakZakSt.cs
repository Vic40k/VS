using System;
using System.Collections.Generic;

// Code scaffolded by EF Core assumes nullable reference types (NRTs) are not used or disabled.
// If you have enabled NRTs for your project, then un-comment the following line:
// #nullable disable

namespace VS_CRM.Models
{
    public partial class ZakZakSt
    {
        public ZakZakSt()
        {
            ZakZakArtic = new HashSet<ZakZakArtic>();
        }

        public DateTime? Data { get; set; }
        public DateTime? DataCh { get; set; }
        public DateTime? DataSend { get; set; }
        public DateTime? Data1 { get; set; }
        public DateTime? Datsavesql { get; set; }
        public string Fio { get; set; }
        public int? Idizgot { get; set; }
        public int Idrec { get; set; }
        public string Ist { get; set; }
        public decimal? Kod { get; set; }
        public bool? Log { get; set; }
        public string Mail { get; set; }
        public string Mem { get; set; }
        public decimal? Month { get; set; }
        public string NSub { get; set; }
        public bool? Nak { get; set; }
        public string NamZak { get; set; }
        public string Order { get; set; }
        public decimal? Prizn { get; set; }
        public bool? Spec { get; set; }
        public bool? Stor { get; set; }
        public decimal? Year { get; set; }
        public bool Delrecord { get; set; }
        public string User { get; set; }
        public string NBase { get; set; }
        public string ClientCode { get; set; }
        public int? WeekNumber { get; set; }
        public int? BaseIdrec { get; set; }
        public int SourceId { get; set; }
        public bool? PaintControl { get; set; }
        public bool? LockControl { get; set; }
        public bool? MetalControl { get; set; }
        public bool? PackControl { get; set; }
        public byte? LabelLang { get; set; }

        public virtual ICollection<ZakZakArtic> ZakZakArtic { get; set; }
    }
}
